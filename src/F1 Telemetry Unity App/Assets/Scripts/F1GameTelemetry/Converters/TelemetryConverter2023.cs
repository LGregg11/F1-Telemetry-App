...
#nullable enable
    ...
    public class TelemetryConverter2023 : BaseTelemetryConverter
    {
        ...
        public TelemetryConverter2023() : base()
        {
        }

        public override string Name => "F1 2023 Telemetry Converter";
        public override GameVersion GameVersion => GameVersion.F12023;
        public override int PacketHeaderSize => 29;
        ...
        public override object? ConvertBytesToStandardPacket(PacketId packetType, byte[] bytes)
        {
            return packetType switch
            {
                ...
                PacketId.TyreSets => null,
                PacketId.MotionEx => null,
                _ => throw new NotImplementedException($"Unknown PacketId: {Enum.GetName(typeof(PacketId), packetType)}")
            };
        }
        ...
        SessionHistory ConvertBytesToSessionHistoryPacket(byte[] bytes)
        {
            ...
            for (int i = 0; i < _LAP_HISTORY_MAX; i++)
            {
                ...
                lapHistoryData[i] = new LapHistoryData(
                    ...
                    packet.lapHistoryData[i].sector1TimeMS,
                    packet.lapHistoryData[i].sector2TimeMS,
                    packet.lapHistoryData[i].sector3TimeMS
                    );
            }

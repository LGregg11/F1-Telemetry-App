...
public class TelemetryConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        if (value.GetType() == typeof(int))
            ...
            return ToTelemetryTime((uint)value);
        else if (value.GetType() == typeof(ushort))
            return ToTelemetryTime((ushort)value);
        return string.Empty;
    }
    ...
    /// <param name="time">Time given in seconds.</param>
    /// <returns>The time in a standard telemetry format.</returns>
    public static string ToTelemetryTime(uint milliseconds)
    {
        double time = System.Convert.ToDouble(milliseconds);
        if (time <= 0)
            return "--:--";
    ...
    /// <param name="time">Time given in seconds.</param>
    /// <returns>The time in a standard telemetry format.</returns>
    public static string ToTelemetryTime(ushort milliseconds)
    {
        double time = System.Convert.ToDouble(milliseconds);
        if (time <= 0)
            return "--:--";
